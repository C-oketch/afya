...
$app2=DB::table('appointments')->select('id')
->where('afya_user_id', $afyauserId)->orderby('id','Desc')->first();
$app2_id =$app2->id;

$app_date=DB::table('appointments')->select('created_at')->where('id', $app_id)->first();

$timestamp = strtotime($app_date->created_at);
$date1= date("jS", $timestamp);
$date2= date("D M y", $timestamp);
// dd($date1,$date2);
?>
@if($app2_id == $app_id)
<li class="active"><a href="#"><i class="fa fa-list"></i><span> TODAY VISIT</span> </a></li>
@else
<li class="active"><a href="#"><i class="fa fa-outdent"></i><span>VISIT {{$date1  .  $date2}}</span> </a></li>
<li class="active"><a href="{{route('patienthistory',$app2_id)}}"><i class="fa fa-angle-double-left"></i><span>BACK</span> </a></li>
