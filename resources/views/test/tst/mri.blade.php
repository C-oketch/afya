...
@section('content')
<?php
$afya_user_id=$Mtests->afya_user_id;
  $dependant_id=$Mtests->dependant_id;
  $ptid=$Mtests->ptid;


if ($Mtests->dependant_id =='Self') {
  $name= $Mtests->firstname.''.$Mtests->secondName;
  $gender=$Mtests->gender;
  $dob=$Mtests->dob;

}else{
  $name= $Mtests->depname1.''.$Mtests->depname2;
  $gender=$Mtests->depgender;
  $dob=$Mtests->depdob;
...
<?php
$i=1;?>
<?php  $cttests = DB::table('mri_tests')->get();  ?>
...
<input type="hidden" class="form-control" id="dep" name="dependant_id" >
<input type="hidden" class="form-control" id="afya" name="afya_user_id" >
</div>
</div>
<div class="form-group">
