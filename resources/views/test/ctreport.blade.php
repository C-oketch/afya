...
<?php
	...
	$findings = DB::table('ct_scan_findings')
->whereNotExists(function($query)use($rtdid)
		{
				$query->select(DB::raw(1))
							->from('radiology_test_result')
							->where('radiology_td_id', '=',$rtdid)
							->whereRaw('ct_scan_findings.id = radiology_test_result.findings_id');
					 })
->where('ct_scan_findings.ct_scan_id', '=',$ctid)
->select('ct_scan_findings.id','ct_scan_findings.findings',
 'ct_scan_findings.results')
->get();
	?>
	<div class="row">
		<div class="col-md-11">
	    <div class="col-md-6">
		     <div class="ibox float-e-margins">
				...
				<div class="ibox-content">
								...
								{{ Form::open(array('route' => array('ctfindings'),'method'=>'POST')) }}
...
<div class="col-md-6">
		<div class="ibox float-e-margins">
				...
				<?php
					$freport = DB::table('ct_scan_findings')
			  ->leftJoin('radiology_test_result', 'ct_scan_findings.id', '=', 'radiology_test_result.findings_id')
        ->where([ ['ct_scan_findings.ct_scan_id', '=',$ctid],['radiology_test_result.radiology_td_id', '=',$rtdid] ])
				->select('radiology_test_result.results','ct_scan_findings.findings')
				->get();
					?>
				<div class="ibox-content">
		...
		<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
										...
										<!-- <p>Sign in today for more expirience.</p> -->
											...
											<label>Technique</label><div class="form-group"><textarea rows="4" name="technique" cols="50" placeholder="How test was done">{{$technique}}</textarea></div>
                     </div>
										<div class="col-md-6">
