            ...
            <div class="ibox-content">
              <table class="table table-bordered">
                ...
                <tr> <td width="15%"><h3>{{$date1}} </h3>{{$date2}} </td>
                  <td width="35%">{{$doct->name}}  <br> {{$doct->FacilityName}} <br>At {{$Time1}} For {{$Time2}} </td>
                  <td width="15%">Next Visit @if($nextvisit)<h3>{{$datenv1}} </h3>{{$datenv2}} @else <br>N/A @endif</td>
                  <td  width="35%"> <p class="pull-right">{{$user->firstname}}  {{$user->secondName}}<br> {{$user->gender}} <br> {{$user->age}}</p></td>
                </tr>
