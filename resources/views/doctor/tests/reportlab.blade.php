...
?>
      ...
      <?php
      $tst21 = DB::table('test_results')->select('value')->where('ptd_id', '=',$tsts1->id)->first();
       ?>
                                         ...
                                         <div class="form-group">
                                           ...
                                           <input type="text" name="radiology" value="{{$tsts1->name}}" class="form-control" readonly/>
                                             ...
                                             <div class="form-group">
                                               ...
                                               <textarea rows="6" name="note" cols="50">@if($tst21){{$tst21->value}} @endif</textarea>
                                             </div>
                                             {{ Form::hidden('cur_appointment_id',$cur_app->id, array('class' => 'form-control')) }}
                                            {{ Form::hidden('appointment_id',$app_id, array('class' => 'form-control')) }}
                                            {{ Form::hidden('ptdid',$tsts1->id, array('class' => 'form-control')) }}

                                            <div class="col-md-12">
                                            <button class=" mtop btn btn-sm btn-primary  m-t-n-xs" type="submit"><strong> @if($tst21) Update  @else Submit @endif</strong></button>
                                            </div>

                                            {{ Form::close() }}
                                 </div>
                                 <div class="col-sm-6"><h4></h4>
                                    {!! Form::open(array('url' => 'doc.labupload','files'=>true)) !!}
                              <input type="hidden" name="_token" value="{{ csrf_token() }}">
       ...
       <?php $images=DB::table('lab_images')->select('id','image')->where('patient_td_id',$tsts1->id)->get();
       ...
       <td><a class="btn btn-danger"  href="{{route('remove.labupload',$image->id)}}"> Remove </a></td>
       </tr>
       @endforeach
       </tbody>
       </table>
