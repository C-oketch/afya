...
<div class="row wrapper white-bg page-heading">
              ...
              <div class="col-lg-6">
                  <div class="title-action">
                      ...
                      <a href="{{route('patienthistory',$cur_app_id)}}"  class="btn btn-primary"><i class="fa fa-angle-double-left"></i> BACK </a>
                      <!-- <a href="{{route('doctor.edithistory',$user->appid)}}"  class="btn btn-primary"><i class="fa fa-edit"></i> EDIT </a> -->
            <input name="b_print" type="button" class="btn btn-primary"   onClick="printdiv('div_print');" value="PRINT">
            <a href="{{url('doctor.slideshow2',$afyauserId)}}"  class="btn btn-primary"><i class="fa fa-edit"></i> SLIDE SHOW </a>
...
<tr> <td width="10%"><h3> {{$date1}}</h3> {{$date2}}</td>
  ...
  <td  width="35%"> <p class="pull-right">{{$name}} <br> {{$gender}} <br> {{$age}}</p></td>
</tr>
...
<strong> {{$ptdetails->rbs}}mmol/l </strong>
  ...
  <tr>
   ...
   @if($family_planning)<span class="mr-1">{{$family_planning->parity}}</span> @endif
   @if($family_planning)<span class="mr-1">{{$family_planning->family_planning}}</span> @endif
...
@if($smoking)
<tr>
 <td colspan="4"><strong>Social History</strong><br>
   <div class="col-md-4">
 @if($smoking)<span class="mr-1"> A smoker</span><span class="pull-right">{{$smoking->smoker}}</span> @endif
 @if($smoking->smoker == 'YES')<span class="mr-1"> cigarretes Per-day</span><span class="pull-right">{{$smoking->cigarretes_per_day}}</span> @endif<br>
 @if($smoking->smoker == "NO")<span class="mr-1"> Ever smoked</span><span class="pull-right">{{$smoking->ever_smoked}}</span> @endif
 @if($smoking->stop_date)<span class="mr-1"> Stop Date</span><span class="pull-right">{{$smoking->stop_date}}</span> @endif
 @if($smoking->period_smoked)<span class="mr-1"> Period Smoked</span><span class="pull-right">{{$smoking->period_smoked}} years</span> @endif
</div>


  @if($alcohol)<div class="col-md-4">
 <span class="mr-1"> Drink Alcohol</span><span class="pull-right">{{$alcohol->drink}}</span><br>
 @if($alcohol->drink == "YES")<span class="mr-1"> Frequency</span><span class="pull-right">{{$alcohol->drinking_frequency}}</span> <br>@endif
<span class="mr-1"> Recreational Drugs</span><span class="pull-right">{{$alcohol->used_recreational_drugs}}</span> <br>
 @if($alcohol->drug_type)<span class="mr-1"> Type</span><span class="pull-right">{{$alcohol->drug_type}}</span> <br>@endif
<span class="mr-1"> Caffeine liquids </span><span class="pull-right">{{$alcohol->caffeine_liquids}} </span>
</div>@endif
 </td>
</tr>
@endif
...
<p><strong> {{$test}} </strong>  @if($result) {{$result}} @endif</p>
...
@if($cardiac)
<tr>
<td colspan="4"><strong>Cardiac Tests :</strong><br>
@foreach($cardiac as $prc)
<p><strong class="mr-01">{{$prc->name}}</strong> {{$prc->results}} </p>
@endforeach</td>
</tr>
@endif
@if($neurology)
<tr>
<td colspan="4"><strong>Neurology Tests :</strong><br>
@foreach($neurology as $prc)
<p><strong class="mr-01">{{$prc->name}}</strong> {{$prc->results}} </p>
@endforeach</td>
</tr>
@endif
...
<td colspan="4"><strong>DIAGNOSIS</strong><br>
 ...
 <?php    $i=1; ?>
 <tr>
 <td colspan="4"><strong>Previous Diagnosis</strong><br>
    @foreach($prevdiagnosis as $tst1)
   <strong class="mr-01">{{$i}}</strong>{{$tst1->disease_id}} <br>
   <?php $i++; ?>
   @endforeach</td>
 </tr>
